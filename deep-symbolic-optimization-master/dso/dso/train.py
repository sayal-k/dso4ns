...
class Trainer():
    def __init__(self, sess, policy, policy_optimizer, gp_controller, logger,
                 ...
                 b_jumpstart=False, early_stopping=True, debug=0, valid_path=None,
                 use_memory=False, memory_capacity=1e3,  warm_start=None, memory_threshold=None,
                 complexity="token", complexity_limit=10,
                 const_optimizer="scipy", const_params=None,  n_cores_batch=1):
        ...
        test_files = [ str(path) for path in Path(valid_path).glob("*.csv") ]
        
        self.X_valid, self.y_valid, _ = get_data(test_files) 
    ...
    def run_one_step(self, override=None):
        ...
        def clone_acc(y_hat):
            y_hat[y_hat >= 0] = 1.0
            y_hat[y_hat < 0] = -1.0
            return np.mean(self.y_valid == y_hat)
        ...
        if r_max == self.r_best:
            y_hat = np.array([p.execute(self.X_valid) for p in programs])
            acc = np.apply_along_axis(clone_acc, 1, y_hat)
            acc_max = np.max(acc)
            p_a = programs[np.argmax(acc)]
            if acc_max > self.acc_best and p_a.complexity < complexity_limit:
                self.acc_best = acc_max
                self.p_a_best = p_a
        ...
        if self.early_stopping and (self.p_r_best.evaluate.get("success") or self.iteration > 300) :
